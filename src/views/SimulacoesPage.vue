<template>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th style="text-align: center; background-color: #14141f;">Localização</th>
            <th style="text-align: center; background-color: #14141f;">Valor</th>
          ...
          <tr>
            ...
            <th><input type="text" v-model="filters.id" style="text-align: center;" placeholder="Telefone" /></th>
            <th></th>
            <th></th>
            <th></th>
          </tr>
        ...
        <tbody>
          <tr v-for="offer in filteredOffers" :key="offer.id">
            <td style="text-align: center;">{{ offer.id }}</td>
            <td>{{ offer.name }}</td>
            <td>{{ offer.email }}</td>
            <td style="text-align: center;">{{ offer.phone }}</td>
            <td style="text-align: center;">{{ offer.city }}, {{ offer.state }}</td>
            <td style="text-align: center;">{{ offer.formattedCredit }}</td>
  ...
  export default {
    ...
    computed: {
      filteredOffers() {
        return this.offers.filter(offer => {
          return (
            (this.filters.id ? offer.id.toString().includes(this.filters.id) : true) &&
            ...
            (this.filters.phone ? offer.phone.toString().includes(this.filters.phone) : true) &&
            (this.filters.city ? offer.city.includes(this.filters.city) : true) &&
            (this.filters.state ? offer.state.includes(this.filters.state) : true) &&
            (this.filters.formattedCredit ? offer.formattedCredit.includes(this.filters.formattedCredit) : true)
          );
        });
      }
    },
    ...
    this.loading = true;
    try {
        const response = await fetch(`https://localhost:7290/api/simulacoes?pageNumber=${this.currentPage}&pageSize=${this.itemsPerPage}`);
      ...
      async showDetails(offerId) {
        try {
          const response = await fetch(`https://localhost:7290/api/simulacoes/${offerId}`);
          const data = await response.json();
